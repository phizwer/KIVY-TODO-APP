    ...
    def mark(self, check, the_list_item):
        ...
        if check.active == True:
            ...
            the_list_item.text = str(
                db.mark_task_as_incomplete(the_list_item.pk))  # Here
...
class MainApp(MDApp):
    ...
    def add_task(self, task, task_date):
        ...
        self.root.ids['container'].add_widget(ListItemWithCheckbox(
            pk=created_task[0], text='[b]'+created_task[1]+'[/b]', secondary_text=created_task[2]))  # Here
        task.text = ''

        # # print(task.text, task_date)
        # self.root.ids['container'].add_widget(ListItemWithCheckbox(
        #     text='[b]'+task.text+'[/b]', secondary_text=task_date))
